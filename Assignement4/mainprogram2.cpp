/* Athabasca University
  ...
  the scene is showing a red square reflecting on a gray surface and also show the shadow of the the red square on the reflective gray surface   
...
//  Sets the background colour of the display window.
...
vector3 rayTracing(vector3 incidentray1, vector3 incidentray2, int tracercalls) {
	...
	if ((wallintersec.x >= -25.0) && (wallintersec.x <= 25.0) && (wallintersec.y >= 0.0)
		...
		vector3 shadowIntersect = wall.intersect(incidentray1, lightPosition);

		if ((shadowIntersect.x >= -25.0) && (shadowIntersect.x <= 25.0) && (shadowIntersect.y >= 0.0)
			&& (shadowIntersect.y <= 50.0)) {

			return grey.scalar(ambient_coef); //return the grey colour for the shadow 
